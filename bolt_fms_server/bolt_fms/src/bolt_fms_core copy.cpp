...
enum class RobotType { MOBILE, ARM };
...
public:
  RobotStateManager(const std::vector<RobotInfo>& robots)
    : Node("robot_state_manager"), tcp_running_(true) {
...
private:
    ...
    for (const auto& [id, type] : robot_types_) {
      if (type == RobotType::MOBILE) {
        ...
        j[id] = {
          {"type", "mobile"},
          {"battery", s.battery},
          {"pose", {{"x", s.pose.position.x}, {"y", s.pose.position.y}, {"z", s.pose.position.z}}},
          {"velocity", s.velocity}
        };
      } else {
  ...
  void run_tcp_server() {
    ...
    if (server_fd < 0) {
      RCLCPP_ERROR(this->get_logger(), "소켓 생성 실패"); return;
    }
    ...
    if (bind(server_fd, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
      RCLCPP_ERROR(this->get_logger(), "바인드 실패"); close(server_fd); return;
    }

    if (listen(server_fd, 5) < 0) {
      RCLCPP_ERROR(this->get_logger(), "리스닝 실패"); close(server_fd); return;
    }
    ...
    while (tcp_running_) {
      ...
      if (client_fd < 0) {
        RCLCPP_WARN(this->get_logger(), "클라이언트 연결 실패"); continue;
      }
